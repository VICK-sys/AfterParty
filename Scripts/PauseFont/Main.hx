...
class FontState extends FlxState
{
    override public function create():Void
    {
        ...
        var output = Sys.getEnv("UNITY_PARTY_PAUSE_FONT_PATH");
        if (output == null) throw "Set UNITY_PARTY_PAUSE_FONT_PATH to the pause menu asset directory.";
        var atlas = new BitmapData(512, 256, true, 0);
        var glyphs = [];
        var text = new FlxText(0, 0, 0, "X", 16);
        text.setFormat("assets/vcr.ttf", 16);
        text.drawFrame(true);
        ...
        File.saveBytes(output + "/vcr16.png", atlas.encode(atlas.rect, new PNGEncoderOptions()));
        File.saveContent(output + "/vcr16.json", haxe.Json.stringify({size:16, advance:advance, lineHeight:lineHeight, glyphs:glyphs}, null, "  ") + "\n");
        Sys.exit(0);
    }
}
